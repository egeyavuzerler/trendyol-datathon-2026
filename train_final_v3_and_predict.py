...
print("\ntraining_features_v3.csv okunuyor...")
df = pd.read_csv(f"{DATA_DIR}/training_features_v3.csv")
...
for fold, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups)):
    ...
    model.save_model(f"{DATA_DIR}/lgbm_v3_fold{fold}.txt")
    models.append(model)
...
print("\nSınıflandırma raporu (OOF, v3 model):")
...
print("\n=== ORTALAMA FEATURE IMPORTANCE (v3 model) ===")
print(avg_importance.to_string(index=False))

with open(f"{DATA_DIR}/best_threshold_v3.txt", "w") as f:
    f.write(str(best_threshold))

print("\nsubmission_features_v3.csv okunuyor...")
...
OUT_PATH = f"{DATA_DIR}/submission_v3.csv"
