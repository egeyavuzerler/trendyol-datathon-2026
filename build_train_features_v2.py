...
# ============================================================
# Yardımcı fonksiyonlar: fit/load TF-IDF ve BM25 matrisleri
# ============================================================

def fit_or_load_tfidf(texts, name, max_features=200_000, min_df=2, max_df=0.3):
    vec_path = f"{DATA_DIR}/tfidf_vectorizer_{name}.joblib"
    mat_path = f"{DATA_DIR}/item_tfidf_matrix_{name}.npz"
    if os.path.exists(vec_path) and os.path.exists(mat_path):
        print(f"  [{name}] kaydedilmiş TF-IDF yükleniyor...")
        vectorizer = joblib.load(vec_path)
        matrix = sparse.load_npz(mat_path)
    else:
        print(f"  [{name}] TF-IDF fit ediliyor...")
        vectorizer = TfidfVectorizer(max_features=max_features, min_df=min_df, max_df=max_df, sublinear_tf=True)
...
def fit_or_load_bm25(texts, name, max_features=200_000, min_df=2, max_df=0.3, k1=BM25_K1, b=BM25_B):
    cv_path = f"{DATA_DIR}/bm25_countvec_{name}.joblib"
    mat_path = f"{DATA_DIR}/bm25_matrix_{name}.npz"
    if os.path.exists(cv_path) and os.path.exists(mat_path):
        print(f"  [{name}] kaydedilmiş BM25 matrisi yükleniyor...")
        count_vec = joblib.load(cv_path)
        bm25_matrix = sparse.load_npz(mat_path)
        return count_vec, bm25_matrix

    print(f"  [{name}] BM25 matrisi hesaplanıyor...")
    count_vec = CountVectorizer(max_features=max_features, min_df=min_df, max_df=max_df)
    tf = count_vec.fit_transform(texts).tocsr()  # (n_docs x vocab), ham terim frekansı

    n_docs = tf.shape[0]
    df = np.asarray((tf > 0).sum(axis=0)).ravel()  # her terimin geçtiği doküman sayısı
    idf = np.log((n_docs - df + 0.5) / (df + 0.5) + 1.0)

    doc_len = np.asarray(tf.sum(axis=1)).ravel()
    avgdl = doc_len.mean() if doc_len.mean() > 0 else 1.0
    denom_per_doc = k1 * (1 - b + b * (doc_len / avgdl))  # her doküman için sabit payda terimi

    tf_csr = tf.tocsr()
    # Her nonzero eleman için satır/sütun indekslerini çıkar
    row_indices = np.repeat(np.arange(tf_csr.shape[0]), np.diff(tf_csr.indptr))
    col_indices = tf_csr.indices
    tf_values = tf_csr.data

    idf_per_nz = idf[col_indices]
    denom_per_nz = denom_per_doc[row_indices]

    weight = idf_per_nz * (tf_values * (k1 + 1)) / (tf_values + denom_per_nz)
    bm25_matrix = sparse.csr_matrix((weight, (row_indices, col_indices)), shape=tf_csr.shape)

    joblib.dump(count_vec, cv_path)
    sparse.save_npz(mat_path, bm25_matrix)
    return count_vec, bm25_matrix


def row_wise_dot(query_vectors_full, item_vectors_full, row_qidx, row_iidx):
    """Her satır için query_vector[i] . item_vector[i] hesaplar (elementwise çarpıp toplama)."""
...
# ============================================================
# 2) Alan-bazı TF-IDF + BM25 matrislerini fit/load et
# ============================================================
print("\nTF-IDF matrisleri hazırlanıyor:")
vec_raw, mat_raw = fit_or_load_tfidf(items["raw_text"].values, "raw")
vec_title, mat_title = fit_or_load_tfidf(items["title"].values, "title")
vec_category, mat_category = fit_or_load_tfidf(items["category_text"].values, "category", max_features=5000, min_df=1, max_df=0.5)
vec_attrs, mat_attrs = fit_or_load_tfidf(items["attributes_raw"].values, "attrs")

print("\nBM25 matrisi hazırlanıyor:")
count_vec_raw, bm25_mat_raw = fit_or_load_bm25(items["raw_text"].values, "raw")

print(f"Tüm matrisler hazır ({time.time()-t0:.1f}s geçti)")

# ============================================================
# 3) training_full.csv + terms.csv + items birleştir
# ============================================================
print("\ntraining_full.csv ve terms.csv okunuyor...")
...
# ============================================================
# 4) Query'leri her alan için transform et (unique term bazında, verimlilik için)
# ============================================================
print("\nQuery'ler transform ediliyor (her alan/skor için)...")
...
print(f"Query transformları tamam ({time.time()-t0:.1f}s geçti)")
...
# ============================================================
# 5) Metin/kategori/attribute eşleşme feature'ları (satır satır)
# ============================================================
print("\nMetin/kategori/attribute eşleşme feature'ları hesaplanıyor...")
...
OUT_PATH = f"{DATA_DIR}/training_features_v2.csv"
features.to_csv(OUT_PATH, index=False)
print(f"\nKaydedildi: {OUT_PATH}  (toplam süre: {time.time()-t0:.1f}s)")
print(features.shape)

print("\nLabel'a göre ortalama feature değerleri:")
numeric_cols = [c for c in features.columns if c not in ("id", "term_id", "item_id", "label")]
print(features.groupby("label")[numeric_cols].mean().T)